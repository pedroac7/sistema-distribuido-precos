...
public class HttpRepositorioServer implements BusinessServer {
    ...
    private void handleClient(Socket client) {
        try (Socket socket = client) {
            ...
            try {
                ...
                System.out.println("[Repositorio] Requisicao HTTP recebida: " + request.method() + " " + request.path());

                HttpResult result = processRequest(request);
                writeJsonResponse(socket.getOutputStream(), result.statusCode(), result.body());
                System.out.println("[Repositorio] Resposta HTTP enviada: " + result.statusCode() + " - " + result.mensagem());
